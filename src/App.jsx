...
const BUSINESS_TYPES = [\
  \{ id: "construction",  label: "Construction / Trades",     icon: "\uc0\u55356 \u57303 \u65039 " \},\
  \{ id: "hvac",          label: "HVAC / Clima",              icon: "\uc0\u55356 \u57132 \u65039 " \},\
  \{ id: "roofing",       label: "Roofing",                   icon: "\uc0\u55356 \u57312 " \},\
  \{ id: "drywall",       label: "Drywall",                   icon: "\uc0\u55358 \u56817 " \},\
  \{ id: "electrical",    label: "Electricista",              icon: "\uc0\u55357 \u56588 " \},\
  \{ id: "plumbing",      label: "Plomer\'eda",                  icon: "\uc0\u55357 \u57023 " \},\
  \{ id: "landscaping",   label: "Landscaping / Lawn Care",   icon: "\uc0\u55356 \u57151 " \},\
  \{ id: "cleaning",      label: "Cleaning / Janitorial",     icon: "\uc0\u55358 \u56825 " \},\
  \{ id: "food_events",   label: "Food Events / Catering",    icon: "\uc0\u55356 \u57213 \u65039 " \},\
  \{ id: "restaurant",    label: "Restaurant",                icon: "\uc0\u55356 \u57204 " \},\
  \{ id: "trucking",      label: "Trucking / Transportation", icon: "\uc0\u55357 \u56987 " \},\
  \{ id: "property_mgmt", label: "Property Management",       icon: "\uc0\u55356 \u57314 " \},\
  \{ id: "barbershop",    label: "Barber\'eda / Sal\'f3n",          icon: "\uc0\u55357 \u56455 " \},\
  \{ id: "general",       label: "General Services",          icon: "\uc0\u9881 \u65039 " \},\
  \{ id: "bookkeeping",   label: "Bookkeeping / Tax Prep",     icon: "\uc0\u55357 \u56530 " \},\
];\
...
// \uc0\u9472 \u9472  CAMBIO 1: "COGS - Materials" \u8594  "Materials" en tradesMat \u9472 \u9472 \
...
// \uc0\u9472 \u9472 \u9472  BANK PROMPTS LIBRARY \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \
const BANK_PROMPTS = \{\
...
Read BOTH columns left AND right on every single row \'97 do NOT skip the right column.\
Each row contains TWO separate checks \'97 extract BOTH.\
Some checks have NO check number \'97 use CHECK #UNKNOWN.\
Asterisk (*) after check number means skipped sequence \'97 include it anyway.\
Amounts use comma formatting like 40,005.00 \'97 parse correctly.\
Use the DATE shown next to each check individually.\
AMOUNT must be negative.\
OUTPUT: Raw CSV \'97 TYPE,DATE,AMOUNT,CONCEPT. No headers. No markdown.`,\
  bank_of_oklahoma: `You are a STRICT extraction agent for BANK OF OKLAHOMA (BOK) statements.\
BOK STATEMENT STRUCTURE \'97 READ IN THIS ORDER:\
1. DEPOSITS section: simple list, DATE on left, AMOUNT on right. Extract ALL.\
2. WITHDRAWALS section: simple list, DATE on left, AMOUNT on right. Extract ALL \'97 this section is LONG, do NOT stop early.\
3. CHECKS section: TWO-COLUMN table \'97 Date | Number | Amount || Date | Number | Amount\
   Read BOTH columns on EVERY row. Each row = TWO separate checks.\
   Asterisk (*) before number = skipped sequence \'97 still extract.\
   Use the DATE shown next to each check individually.\
4. DAILY ACCOUNT BALANCE section: NOT transactions \'97 skip entirely.\
5. STOP \'97 Everything after the DAILY ACCOUNT BALANCE section is check and deposit IMAGES.\
   These image pages contain scanned photos of physical checks and deposit slips.\
   They show dollar amounts but these are NOT transactions \'97 they are already captured above.\
   ...
   EXTRACTING FROM IMAGE PAGES WILL CREATE DUPLICATES \'97 strictly forbidden.\
DEPOSITS positive. WITHDRAWALS and CHECKS negative.\
Dates format MM-DD-YY \uc0\u8594  convert to MM/DD/YYYY.\
...
1. DEPOSITS section: Simple list \'97 DATE, DESCRIPTION, AMOUNT (positive). Extract ALL.\
2. ELECTRONIC CREDITS section: Also deposits \'97 DATE, DESCRIPTION, AMOUNT (positive). Extract ALL.\
3. ELECTRONIC DEBITS section: Withdrawals \'97 each transaction has 3 lines:\
   Line 1: DATE and MERCHANT NAME and AMOUNT (negative, shown as -$XX.XX)\
   Lines 2-3: transaction details \'97 use merchant name from line 1 as concept.\
   Extract ALL \'97 this section is very long, do NOT stop early.\
4. OTHER DEBITS section: Also withdrawals \'97 extract all (service charges etc.)\
5. CHECKS CLEARED section: THREE-COLUMN table:\
   ...
   Check number 0 means no check number \'97 use CHECK #UNKNOWN.\
   Asterisk (*) after number means skipped sequence \'97 still extract.\
   Use the DATE shown next to each check.\
6. CHECK IMAGE PAGES \'97 CRITICAL: Pages showing scanned check photos have text lines\
   formatted as "#NNNN MM/DD/YYYY $X,XXX.XX" below each image (two per row).\
   These are NOT transactions \'97 they are ALREADY captured in the CHECKS CLEARED table.\
   If this PDF (or PDF portion) contains ONLY these "#NNNN date amount" lines with\
   no section headers (Deposits, Electronic Credits, Electronic Debits, Checks Cleared),\
   it is an image-only portion: extract NOTHING. Output must be empty.\
7. IGNORE the "Daily Balances" section \'97 those are balances, not transactions.\
8. IGNORE "THIS PAGE LEFT INTENTIONALLY BLANK" and marketing/advertisement pages.\
DEPOSITS positive. ELECTRONIC DEBITS amounts already have minus sign \'97 keep negative.\
OUTPUT: Raw CSV \'97 TYPE,DATE,AMOUNT,CONCEPT. No headers. No markdown.`,\
  bank_of_america: `You are a STRICT extraction agent for BANK OF AMERICA statements.\
BANK OF AMERICA STRUCTURE:\
1. DEPOSITS AND OTHER CREDITS section: Simple list \'97 Date, Description, Amount (positive). Extract ALL.\
2. WITHDRAWALS AND OTHER DEBITS section: Has two parts:\
   - Direct debits (Zelle, ACH, online payments) \'97 negative amounts\
   - Card account section (grouped under "Card account # XXXX") \'97 also negative amounts\
   Extract ALL from both parts \'97 do NOT stop at the subtotal line.\
3. SERVICE FEES section: Extract any fees as withdrawals.\
4. CHECKS section: TWO-COLUMN table \'97 Date | Check # | Amount || Date | Check # | Amount\
   Read BOTH columns on EVERY row. Each row = TWO separate checks.\
   Extract ALL checks.\
5. IGNORE pages marked "This page intentionally left blank".\
6. IGNORE check images pages \'97 those are NOT transaction data.\
Deposits are positive. Withdrawals shown as negative \'97 keep negative.\
Dates format MM/DD/YY \'97 convert to MM/DD/YYYY.\
"Subtotal for card account" and "Total" lines are NOT transactions \'97 skip them.\
OUTPUT: Raw CSV \'97 TYPE,DATE,AMOUNT,CONCEPT. No headers. No markdown.`,\
...
FECHA = Date, DESCRIPCI\'d3N = Description, CANTIDAD = Amount\
CHASE BANK STRUCTURE:\
1. DEP\'d3SITOS Y ADICIONES (DEPOSITS AND ADDITIONS): All are deposits \'97 positive amounts. Extract ALL.\
2. RETIROS ELECTR\'d3NICOS (ELECTRONIC WITHDRAWALS): All are withdrawals.\
   Amounts appear as POSITIVE numbers in statement but output them as NEGATIVE.\
3. CARGOS (FEES/CHARGES): Also withdrawals \'97 output as negative.\
4. NO CHECKS in this statement type typically.\
5. IGNORE blank pages ("Esta p\'e1gina se ha dejado en blanco intencionalmente").\
Dates shown as MM/DD \'97 convert to MM/DD/YYYY using the statement year.\
Each transaction may span multiple lines \'97 use first line description as concept.\
"Total de dep\'f3sitos" and "Total de retiros" are summary lines \'97 NOT transactions, skip them.\
SALDO FINAL DIARIO section \'97 NOT transactions, skip entirely.\
OUTPUT: Raw CSV \'97 TYPE,DATE,AMOUNT,CONCEPT. No headers. No markdown.`,\
  msu_federal_credit_union: `You are a STRICT extraction agent for MSU FEDERAL CREDIT UNION statements.\
\
\uc0\u9888 \u65039  FIRST \'97 READ THIS BEFORE ANYTHING ELSE:\
SCAN THE VERY FIRST LINE OF THIS PDF.\
If you see any text containing the word "(Continued)" \'97 for example:\
  "SMALL BUSINESS CHECKING - 83 (Continued)"\
  "SPARTAN SAVER (Continued)"\
  "BUSINESS IMMA (Continued)"\
\uc0\u8594  IMMEDIATELY identify the sub-account named before "(Continued)" and set it as the ACTIVE sub-account.\
\uc0\u8594  Apply its prefix ([CHECKING], [SAVER], or [IMMA]) to EVERY transaction in this PDF from the very first line.\
\uc0\u8594  Do NOT wait for a full header. The "(Continued)" line IS the header for this PDF portion.\
\
Sub-account \uc0\u8594  prefix mapping for (Continued) detection:\
  "SMALL BUSINESS CHECKING" \uc0\u8594  [CHECKING]\
  "SPARTAN SAVER" or "BUSINESS SPARTAN SAVER" \uc0\u8594  [SAVER]\
  "BUSINESS IMMA" or "IMMA" \uc0\u8594  [IMMA]\
\
If the first line does NOT contain "(Continued)" and there is NO sub-account header before the first transaction,\
default to [CHECKING] immediately \'97 do not leave any transaction without a prefix.\
\
MSU FCU STATEMENT STRUCTURE \'97 CRITICAL RULES:\
\
...
\
2. SUB-ACCOUNT TRACKING \'97 CRITICAL:\
   As you read the PDF top-to-bottom, track the ACTIVE sub-account by watching for these header lines:\
   - "BUSINESS SPARTAN SAVER" or "SPARTAN SAVER" \uc0\u8594  active sub-account = SAVER \u8594  prefix = [SAVER]\
   - "BUSINESS IMMA" or "IMMA" \uc0\u8594  active sub-account = IMMA \u8594  prefix = [IMMA]\
   - "SMALL BUSINESS CHECKING" \uc0\u8594  active sub-account = CHECKING \u8594  prefix = [CHECKING]\
...
\
6. CLEARED CHECK SUMMARY table: DO NOT extract \'97 already captured as Draft lines above.\
   Also: DO NOT extract from "Withdrawals and Other Charges" section \'97 already captured above.\
   DO NOT extract from "Deposits and Other Credits" section \'97 already captured above.\
...
7. SUMMARY PAGES ("Withdrawals and Other Charges" / "Deposits and Other Credits"):\
   DO NOT extract from these \'97 they are summaries and will create duplicates.\
\
8. TRANSFERS:\
...
\
9. Dates format: MM-DD \uc0\u8594  convert to MM/DD/YYYY using the statement year shown on page 1.\
   If Trans Date differs from Post Date, use Post Date.\
\
10. DEPOSITS positive. WITHDRAWALS negative.\
11. "Balance Forward" and "Ending Balance" lines are NOT transactions \'97 skip them.\
12. "Dividends Earned YTD" lines are NOT transactions \'97 skip them.\
    Dividend entries WITH an amount and date ARE transactions \'97 include them as DEPOSITS with prefix.\
\
OUTPUT: Raw CSV \'97 TYPE,DATE,AMOUNT,ACCOUNT,CONCEPT. No headers. No markdown. No explanation.\
...
4. Dates format: MM/DD/YYYY or similar \'97 keep as MM/DD/YYYY.\
5. DEPOSITS positive. WITHDRAWALS negative.\
6. Do NOT extract summary lines or balance lines \'97 only actual transactions.\
OUTPUT: Raw CSV \'97 TYPE,DATE,AMOUNT,CONCEPT. No headers. No markdown.`,\
...
2. SAVINGS ACTIVITY section: Same format \'97 extract all transfers.\
...
4. "Total Subtracted/Added" lines are summary rows \'97 NOT transactions, skip them.\
5. "This page has been intentionally left blank" \'97 skip entirely.\
6. Fees section at the top (Monthly Service Fee, ATM Fee) \'97 skip if shown as "Waived",\
   extract only if actually charged with a dollar amount.\
7. Dates format: MM/DD \'97 convert to MM/DD/YYYY using the statement year.\
...
Check tables may have TWO or THREE columns \'97 read ALL columns on every row.\
Summary totals and balance rows are NOT transactions \'97 skip them.\
Blank pages \'97 skip entirely.\
OUTPUT: Raw CSV \'97 TYPE,DATE,AMOUNT,CONCEPT. No headers. No markdown.`\
\};\
const MERCHANT_DICT = [\
  // \uc0\u9472 \u9472  FUEL \u9472 \u9472 \
  ...
  // \uc0\u9472 \u9472  CAMBIO 2 en merchants: "COGS - Materials" \u8594  "Materials" \u9472 \u9472 \
  ...
  // \uc0\u9472 \u9472  CAMBIO 3: NUEVOS MERCHANTS \'97 Payroll Tax, Licenses & Permits, Legal & Professional Services \u9472 \u9472 \
  ...
  ...[["MCDONALD"],["STARBUCKS"],["CHICK-FIL-A","CHICKFILA"],["SUBWAY"],["CHIPOTLE"],["TACO BELL"],["WENDYS","WENDY'S"],["BURGER KING"],["DOMINOS"],["PIZZA HUT"],["POPEYES"],["PANDA EXPRESS"],["IN-N-OUT"],["WHATABURGER"],["RAISING CANE"],["SONIC DRIVE"],["JACK IN THE BOX"],["DAIRY QUEEN"],["FIVE GUYS"],["PANERA"],["DUNKIN"],["BUFFALO WILD","BUFFALO WILD WINGS"],["OLIVE GARDEN"]].map(p=>(\{patterns:p,category:"Meals & Entertainment"\})),\
  ...
  // \uc0\u9472 \u9472  CAMBIO 4: V&M BOOKKEEPING \u8594  Legal & Professional Services \u9472 \u9472 \
  ...
  // \uc0\u9472 \u9472  MERCURY BANK \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \
  \{ patterns:["PERSON PAY LUIS"], category:"Owner Draw" \},\
  \{ patterns:["POS AMAZON.COM","DEBIT CARD AMAZON"], category:"Materials" \},\
  // \uc0\u9472 \u9472  REGLAS APRENDIDAS \'97 vm \'b7 general \'b7 3/4/2026 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \
  ...
  // \uc0\u9472 \u9472  MABREY BANK / CONSTRUCTION (Najera) \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \
  ...
  // \uc0\u9472 \u9472  CHENTILLO CONSTRUCTION (BofA) \u9472 \u9472 \
  ...
  // \uc0\u9472 \u9472  MSU FEDERAL CREDIT UNION (El Chaparrito) \u9472 \u9472 \
  ...
  // \uc0\u9472 \u9472  CITIBANK (James Alfaro \'b7 electrical) \u9472 \u9472 \
...
\}\
function dedupeChecksByNumber(rows) \{\
  const seen = new Set();\
  return rows.filter(r => \{\
    if (r.type !== "WITHDRAWAL") return true;\
    const m = r.concept.toUpperCase().match(/(?:CHECK|CHK|DRAFT)\\s*#?\\s*(\\d\{1,6\})/);\
    if (!m) return true;\
    const num = m[1];\
    // EXCEPCI\'d3N: cheques de caja sin folio (check #0, #00, etc.)\
    // No se deduplican \'97 varios cheques leg\'edtimos pueden compartir el n\'famero 0\
    if (parseInt(num, 10) === 0) return true;\
    const key = num + "|" + Math.abs(parseFloat(r.amount)||0).toFixed(2);\
...
function categorize(concept, amount, isDeposit, businessType, learnedMerchants) \{\
  ...
  if (upper.includes("INTUIT")) \{\
    ...
    if (/INTUIT\\s+\\d\{8,\}/.test(upper))\
      return \{ category:"Payroll & Wages", level:"HARD" \};\
    return \{ category:"ASK TO CLIENT", level:"ASK", reason:"Intuit \'97 \'bfPayroll, QuickBooks o TurboTax?" \};\
  \}\
  if (isDeposit) \{\
  ...
  for (const entry of MERCHANT_DICT) \{\
    if (entry.patterns.some(p => upper.includes(p.toUpperCase()))) \{\
      ...
      if (typeof entry.category === "object") \{\
        ...
        // Si el merchant es gasto pero el dinero entr\'f3 \uc0\u8594  es ingreso\
...
\}\
// \uc0\u9472 \u9472 \u9472  CLAUDE API \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \
...
\}\
// \uc0\u9472 \u9472 \u9472  AGENT 0: DETECT BANK \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \
...
- total_pages: total number of pages in this statement (look for "Page X of Y" or "P\'e1gina X de Y")\
...
- "Mabrey Bank" or "MabreyBank" \uc0\u8594  "mabrey_bank"\
- "Bank of Oklahoma" or "BOK" or "bok.com" \uc0\u8594  "bank_of_oklahoma"\
- "Arvest" or "arvest.com" \uc0\u8594  "arvest_bank"\
- "Bank of America" or "bankofamerica.com" or "BANK OF AMERICA" \uc0\u8594  "bank_of_america"\
- "Chase" or "JPMorgan Chase" or "chase.com" \uc0\u8594  "chase"\
- "MSU Federal Credit Union" or "MSUFCU" or "msufcu.org" or "MSU FCU" or "1-800-MSU-4YOU" \uc0\u8594  "msu_federal_credit_union"\
- "Mercury" or "mercury.com" or "Mercury Bank" \uc0\u8594  "mercury_bank"\
- "Citibank" or "CITIBANK" or "citi.com" or "Citibank N.A." \uc0\u8594  "citibank"\
- Any other bank \uc0\u8594  "unknown"\
...
Example: \{"bank_name":"Mabrey Bank","bank_id":"mabrey_bank","total_pages":16,"period_start":"02/02/2026","period_end":"03/01/2026","total_deposits":75616.02,"total_withdrawals":86217.03\}`;\
  ...
  \}\
\}\
// \uc0\u9472 \u9472 \u9472  AGENT 1: EXTRACT TRANSACTIONS \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \
async function extractTransactions(b64, bankId = "default", continuationHint = "") \{\
  ...
  const skipCheckSummaryRule = bankId === "msu_federal_credit_union"\
    ? "DO NOT extract from the Cleared Check Summary table or any summary/totals pages \'97 those transactions are already in the main ledger as Draft lines and would create duplicates."\
    : 'Also extract ALL checks from any "Cleared Check Summary", "Checks Paid", "Draft Summary", or similar table.';\
  const continuationBlock = continuationHint ? `\\n\uc0\u9888 \u65039  CONTINUATION CONTEXT: $\{continuationHint\}\\n` : "";\
...
\
  ...
  if (bankId === "msu_federal_credit_union") \{\
    ...
    return rows.filter(row => \{\
      // Filtrar filas sin prefijo \'97 vienen de p\'e1ginas de resumen (Withdrawals and Other Charges, etc.)\
      if (!row.concept.startsWith("[CHECKING]") &&\
          !row.concept.startsWith("[SAVER]") &&\
          !row.concept.startsWith("[IMMA]")) return false;\
      // Deduplicar drafts/cheques por n\'famero\
...
\}\
\
// \uc0\u9472 \u9472 \u9472  AGENT 2: EXTRACT BALANCES \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \
...
IMPORTANT \'97 Different banks use different formats:\
MABREY BANK format (page 1 summary box):\
- Look for lines like "X Deposits $XX,XXX.XX" and "X Checks/Debits $XX,XXX.XX"\
- beginning_balance and ending_balance may not be shown \'97 use 0 if not found\
...
- period_start and period_end = from "Statement Period: MM-DD-YY to MM-DD-YY" \uc0\u8594  convert to MM/DD/YYYY\
...
- Beginning balance and Ending balance are shown explicitly \'97 extract both\
CHASE format (page 1 RESUMEN DE CUENTA / ACCOUNT SUMMARY):\
- "Dep\'f3sitos y Adiciones" or "Deposits and Additions" = total_deposits\
- "Retiros Electr\'f3nicos" or "Electronic Withdrawals" = total_withdrawals\
MSU FEDERAL CREDIT UNION format \'97 extract from each sub-account's OWN LEDGER (do NOT use the page 1 summary):\
- This statement has 3 sub-accounts. The page 1 summary is NOT reliable \'97 IGNORE it.\
...
- FOR EACH sub-account:\
  ...
  * total_withdrawals: sum of ALL negative transaction amounts \'97 use absolute value (positive number)\
...
Example: [\{"account_name":"Business Checking","account_number":"1234","beginning_balance":5000.00,"total_deposits":10000.00,"total_withdrawals":8000.00,"ending_balance":7000.00,"period_start":"01/01/2024","period_end":"01/31/2024"\}]`;\
  ...
  \} catch \{ return []; \}\
\}\
// \uc0\u9472 \u9472 \u9472  SECOND PASS \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \
...
No headers, no markdown, no explanation.`;\
  ...
  return rows;\
\}\
// \uc0\u9472 \u9472 \u9472  CHECK SUMMARY PASS \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \
...
AMOUNT must be negative. No headers. No markdown.`;\
  ...
  return rows;\
\}\
// \uc0\u9472 \u9472 \u9472  STORAGE \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \
const sc = async (id) => \{\
...
// \uc0\u9472 \u9472 \u9472  APP \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \
...
\
    // \uc0\u9472 \u9472  AGENT 0: Detect bank \u9472 \u9472 \
    let detectedBank;\
    ...
    \} else \{\
      ...
      setP(`Agente 0: Banco ya identificado \'97 $\{detectedBank.bank_name\}`, 5);\
    \}\
\
    // \uc0\u9472 \u9472  \u55357 \u57000  BLOQUEO: PDF mayor a 6 p\'e1ginas sin Split Mode activo \u9472 \u9472 \
    if ((detectedBank.total_pages || 0) > 6 && !currentSplitMode) \{\
      setBankInfo(detectedBank);\
      setP(`\uc0\u10060  PDF de $\{detectedBank.total_pages\} p\'e1ginas \'97 activa Modo Split`, 100);\
...
\
    ...
    setP(`\uc0\u9989  Banco: $\{detectedBank.bank_name\} \'b7 $\{totalPages\} p\'e1ginas`, 8);\
\
    // \uc0\u9472 \u9472  AGENT 1: Extract transactions \u9472 \u9472 \
    ...
    setP(`\uc0\u9989  $\{rows.length\} transacciones encontradas`, 35);\
\
    // \uc0\u9472 \u9472  AGENT 2: Extract balances \u9472 \u9472 \
    let bals;\
    if (!currentSplitMode || currentPartNum === 1 || isMSUFCU) \{\
      ...
      setP(`\uc0\u9989  $\{bals.length\} cuenta(s) detectada(s)`, 48);\
    \} else \{\
      bals = balancesRef.current.length > 0 ? balancesRef.current : [];\
      setP(`\uc0\u9989  Saldos de Parte 1 conservados \'97 $\{bals.length\} cuenta(s) \'b7 Banco: $\{detectedBank.bank_name\}`, 48);\
    \}\
\
    // \uc0\u9472 \u9472  SMART MULTI-PASS \u9472 \u9472 \
    let allRows = rows;\
    if (bals.length > 0) \{\
      ...
      if (withDiff > 50 && bankId !== "msu_federal_credit_union") \{\
        setP(`\uc0\u9889  Agente 2A: Buscando cheques faltantes ($$\{withDiff.toFixed(0)\})...`, 52);\
        ...
        setP(`\uc0\u9989  Pasada cheques: +$\{newCheckRows.length\} encontrados`, 60);\
      \}\
      ...
      if ((remDepDiff > 50 || remWithDiff > 50) && bankId !== "msu_federal_credit_union") \{\
        setP(`\uc0\u9889  Agente 2B: Buscando transacciones adicionales ($$\{Math.max(remDepDiff,remWithDiff).toFixed(0)\})...`, 63);\
        ...
        setP(`\uc0\u9989  Pasada adicional: +$\{newRows.length\} m\'e1s encontradas`, 68);\
      \}\
    \}\
\
    // \uc0\u9472 \u9472  AGENT 3: Categorize \u9472 \u9472 \
    ...
    \});\
    setP("\uc0\u9989  Categorizaci\'f3n completada", 85);\
    ...
    if (learn && learnKey) \{\
      ...
      \} else if (!cleanKey.toUpperCase().includes("ZELLE")) \{\
        cleanKey = cleanKey\
          ...
          .toUpperCase();\
      \}\
      // No guardar si cleanKey es inv\'e1lido o ya existe en MERCHANT_DICT\
      ...
      if (cleanKey.length < 3 || alreadyKnown) \{\
        // Aplicar categor\'eda sin guardar en memoria\
        const nd = \{ ...clientData \};\
        setClientData(nd);\
      \} else \{\
  ...
  \}\
  function fmtDate(d) \{\
    // Convierte MM/DD/YYYY \uc0\u8594  DD-MM-YYYY\
    if (!d) return d;\
    const parts = d.split("/");\
  ...
  function downloadPnL() \{\
    ...
    // \uc0\u9472 \u9472  CAMBIO 6: COGS_CATS y OPEX_CATS actualizados \u9472 \u9472 \
    ...
    // \uc0\u9472 \u9472  Generar opini\'f3n autom\'e1tica basada en n\'fameros \u9472 \u9472 \
    ...
    if (totalIncome === 0) \{\
      opinion = "\uc0\u9888 \u65039  No se registraron ingresos en este per\'edodo. Verifica que las transacciones est\'e9n correctamente categorizadas.";\
      opinionColor = "#991b1b";\
    \} else if (marginPct >= 40) \{\
      opinion = `\uc0\u9989  Excelente mes. Margen neto del $\{marginPct\}% \'97 el negocio est\'e1 muy saludable. Los gastos operativos est\'e1n bien controlados ($\{opexPct\}% de los ingresos).`;\
      opinionColor = "#166534";\
    \} else if (marginPct >= 20) \{\
      opinion = `\uc0\u55357 \u56397  Buen mes. Margen neto del $\{marginPct\}%. Hay oportunidad de mejorar reduciendo gastos operativos que representan el $\{opexPct\}% de los ingresos.`;\
      opinionColor = "#1a56db";\
    \} else if (marginPct >= 0) \{\
      opinion = `\uc0\u9888 \u65039  Margen neto bajo ($\{marginPct\}%). El negocio es rentable pero ajustado. Revisar gastos operativos ($\{opexPct\}% de ingresos) para mejorar la utilidad.`;\
      opinionColor = "#92400e";\
    \} else \{\
      opinion = `\uc0\u55357 \u57000  Mes con p\'e9rdida neta de $$\{fmt2(Math.abs(netProfit))\}. Los gastos superaron los ingresos. Requiere atenci\'f3n inmediata en control de gastos.`;\
      opinionColor = "#991b1b";\
    \}\
\
    // \uc0\u9472 \u9472  Construir secciones HTML \u9472 \u9472 \
    const detailBtn = (cat) => `<button onclick="openDetail('$\{cat.replace(/&/g,'&amp;')\}')" class="detail-btn">\uc0\u55357 \u56589 </button>`;\
...
\
    const personalRows = PERSONAL_CATS.map(cat => \{\
      ...
      const txRows = txs.map(r =>\
        `<tr><td class="cat" style="padding-left:28px;color:#64748b">$\{fmtDate(r.date)\} \'97 $\{r.concept\}</td><td class="amt" style="color:#64748b">$$\{fmt2(Math.abs(parseFloat(r.amount)||0))\}</td></tr>`\
      ).join("");\
      return `\
...
\
    const emojiMap = \{'Income - Services':'\uc0\u55357 \u56496 ','Other Income':'\u55357 \u56496 ','Refund Received':'\u55357 \u56501 ','Materials':'\u55356 \u57303 \u65039 ','Subcontractor Expense':'\u55357 \u56439 ','COGS - Fuel (Production)':'\u9981 ','Payroll & Wages':'\u55357 \u56508 ','Payroll Tax':'\u55356 \u57307 \u65039 ','Legal & Professional Services':'\u9878 \u65039 ','Licenses & Permits':'\u55357 \u56523 ','Advertising & Marketing':'\u55357 \u56547 ','Bank Fees':'\u55356 \u57318 ','Donations':'\u55357 \u56911 ','Insurance':'\u55357 \u57057 \u65039 ','Loan Payment':'\u55357 \u56499 ','Meals & Entertainment':'\u55356 \u57213 \u65039 ','Office Supplies':'\u55357 \u56526 ','Operating Expenses - Delivery & Postage':'\u55357 \u56558 ','Operating Expenses - Parking':'\u55356 \u56703 \u65039 ','Operating Expenses - Supplies':'\u55357 \u56550 ','Personal Payment':'\u55357 \u56420 ','Pharmacy':'\u55357 \u56458 ','Rent & Lease':'\u55356 \u57312 ','Repairs & Maintenance':'\u55357 \u56616 ','Software & Subscriptions':'\u55357 \u56507 ','Taxes & Licenses':'\u55357 \u56523 ','Telephone & Internet':'\u55357 \u56561 ','Transfer Out':'\u55357 \u56580 ','Transfer In':'\u55357 \u56580 ','Travel & Transportation':'\u9992 \u65039 ','Uniforms':'\u55357 \u56404 ','Utilities':'\u55357 \u56481 ','Vehicle - Fuel (Non-Production)':'\u9981 ','Vehicle - Maintenance':'\u55357 \u56615 ','Owner Draw':'\u55357 \u56420 ','ASK TO CLIENT':'\u10067 '\};\
    const detailPages = \{\};\
    [...new Set(transactions.map(r=>r.category))].filter(Boolean).forEach(cat => \{\
      ...
      const emoji = emojiMap[cat] || '\uc0\u55357 \u56522 ';\
...
<head>\
<meta charset="UTF-8"/>\
<title>P&L \'97 $\{clientData?.name || ""\}</title>\
<style>\
...
<body>\
<div class="no-print">\
  <span class="print-tip">\uc0\u55357 \u56481  Para guardar como PDF: Archivo \u8594  Imprimir \u8594  Guardar como PDF</span>\
  <div style="display:flex;gap:8px;align-items:center">\
    <button id="btn-es" onclick="setLang('es')" style="background:#fff;color:#1a56db;border:none;border-radius:6px;padding:6px 14px;font-weight:700;cursor:pointer;font-size:12px">\uc0\u55356 \u56818 \u55356 \u56829  ES</button>\
    <button id="btn-en" onclick="setLang('en')" style="background:rgba(255,255,255,0.2);color:#fff;border:1px solid rgba(255,255,255,0.4);border-radius:6px;padding:6px 14px;font-weight:700;cursor:pointer;font-size:12px">\uc0\u55356 \u56826 \u55356 \u56824  EN</button>\
    <button onclick="window.print()" style="background:#22c55e;color:#fff;border:none;border-radius:6px;padding:6px 18px;font-weight:700;cursor:pointer;font-size:12px">\uc0\u55357 \u56744 \u65039  PDF</button>\
  </div>\
</div>\
<script>\
const T = \{\
  es: \{\
    ...
    income: "\uc0\u55357 \u56496  Ingresos", cogs: "\u55356 \u57303 \u65039  Costo de Ventas (COGS)", opex: "\u55357 \u56523  Gastos Operativos",\
    ...
    personal: "\uc0\u9888 \u65039  Movimientos Personales y No Empresariales", totalPersonal: "TOTAL MOVIMIENTOS PERSONALES",\
    subtotal: "Subtotal", printTip: "\uc0\u55357 \u56481  Para guardar como PDF: Archivo \u8594  Imprimir \u8594  Guardar como PDF",\
    opinions: \{\
      noIncome: "\uc0\u9888 \u65039  No se registraron ingresos en este per\'edodo. Verifica que las transacciones est\'e9n correctamente categorizadas.",\
      excellent: (m,o) => \\`\uc0\u9989  Excelente mes. Margen neto del \\$\{m\}% \'97 el negocio est\'e1 muy saludable. Los gastos operativos est\'e1n bien controlados (\\$\{o\}% de los ingresos).\\`,\
      good: (m,o) => \\`\uc0\u55357 \u56397  Buen mes. Margen neto del \\$\{m\}%. Hay oportunidad de mejorar reduciendo gastos operativos que representan el \\$\{o\}% de los ingresos.\\`,\
      low: (m,o) => \\`\uc0\u9888 \u65039  Margen neto bajo (\\$\{m\}%). El negocio es rentable pero ajustado. Revisar gastos operativos (\\$\{o\}% de ingresos) para mejorar la utilidad.\\`,\
      loss: (v) => \\`\uc0\u55357 \u57000  Mes con p\'e9rdida neta de $\\$\{v\}. Los gastos superaron los ingresos. Requiere atenci\'f3n inmediata en control de gastos.\\`,\
    \},\
  ...
  en: \{\
    ...
    income: "\uc0\u55357 \u56496  Income", cogs: "\u55356 \u57303 \u65039  Cost of Goods Sold (COGS)", opex: "\u55357 \u56523  Operating Expenses",\
    ...
    personal: "\uc0\u9888 \u65039  Personal & Non-Business Movements", totalPersonal: "TOTAL PERSONAL MOVEMENTS",\
    subtotal: "Subtotal", printTip: "\uc0\u55357 \u56481  To save as PDF: File \u8594  Print \u8594  Save as PDF",\
    opinions: \{\
      noIncome: "\uc0\u9888 \u65039  No income recorded in this period. Please verify transactions are correctly categorized.",\
      excellent: (m,o) => \\`\uc0\u9989  Excellent month. Net margin of \\$\{m\}% \'97 the business is very healthy. Operating expenses are well controlled (\\$\{o\}% of revenue).\\`,\
      good: (m,o) => \\`\uc0\u55357 \u56397  Good month. Net margin of \\$\{m\}%. There's room to improve by reducing operating expenses, which represent \\$\{o\}% of revenue.\\`,\
      low: (m,o) => \\`\uc0\u9888 \u65039  Low net margin (\\$\{m\}%). The business is profitable but tight. Review operating expenses (\\$\{o\}% of revenue) to improve profitability.\\`,\
      loss: (v) => \\`\uc0\u55357 \u57000  Net loss of $\\$\{v\} this month. Expenses exceeded revenue. Immediate attention to cost control is required.\\`,\
    \},\
...
function setLang(lang) \{\
  const t = T[lang];\
  // T\'edtulo y header\
  document.querySelector('h1').textContent = "\uc0\u55357 \u56522  " + t.title;\
...
<div class="wrapper">\
  <div class="header">\
    <h1>\uc0\u55357 \u56522  Profit & Loss Statement</h1>\
    <div class="meta">\
      <span>\uc0\u55357 \u56420  $\{clientData?.name || ""\}</span>\
      <span>\uc0\u55356 \u57318  $\{bankInfo?.bank_name || ""\}</span>\
      <span>\uc0\u55357 \u56517  $\{periodStart\} \'97 $\{periodEnd\}</span>\
      <span>\uc0\u55357 \u56787 \u65039  Generado el $\{today2\}</span>\
    </div>\
  </div>\
  ...
  <div class="body">\
    ...
    <table>\
      <tr class="section-header"><td colspan="2" id="sec-income">\uc0\u55357 \u56496  Ingresos</td></tr>\
      $\{incomeRows\}\
      ...
      <tr class="section-header"><td colspan="2" id="sec-cogs">\uc0\u55356 \u57303 \u65039  Costo de Ventas (COGS)</td></tr>\
      $\{cogsRows\}\
      ...
      <tr class="spacer"><td colspan="2"></td></tr>\
      <tr class="section-header"><td colspan="2" id="sec-opex">\uc0\u55357 \u56523  Gastos Operativos</td></tr>\
      $\{opexRows\}\
    ...
    <table>\
      <tr class="personal-header"><td colspan="2" id="sec-personal">\uc0\u9888 \u65039  Movimientos Personales y No Empresariales</td></tr>\
      $\{personalRows\}\
  ...
  <div class="footer">Generado por el Agente de Mau Bautista \'b7 V&amp;M Bookkeeping Group LLC</div>\
</div>\
</body>\
</html>`;\
\
            ...
            <tbody>\
              $\{rows.map((r,i)=>`\
                <tr class="$\{i===0?'orig':'dup'\}">\
                  ...
                  <td>$\{r.concept\}</td>\
                  <td><span class="tag $\{i===0?'tag-ok':'tag-dup'\}">$\{i===0?"\uc0\u10003  ORIGINAL":"\u9888  DUPLICADO"\}</span></td>\
                </tr>`).join("")\}\
            </tbody>\
          </table>\
        </div>`;\
...
<head>\
<meta charset="UTF-8"/>\
<title>Duplicados \'97 $\{clientName\}</title>\
<style>\
  *\{box-sizing:border-box;margin:0;padding:0\}\
...
<body>\
  <div class="no-print" style="background:#1a56db;color:#fff;padding:10px 16px;border-radius:8px;margin-bottom:16px;display:flex;align-items:center;justify-content:space-between">\
    <span class="print-tip">\uc0\u55357 \u56481  Para guardar como PDF: Archivo \u8594  Imprimir \u8594  Guardar como PDF</span>\
    <button onclick="window.print()" style="background:#fff;color:#1a56db;border:none;border-radius:6px;padding:6px 16px;font-weight:700;cursor:pointer;font-size:12px">\uc0\u55357 \u56744 \u65039  Imprimir / Guardar PDF</button>\
  </div>\
  <div class="header">\
    <h1>\uc0\u55357 \u56523  Reporte de Duplicados</h1>\
    <p>$\{clientName\} \'b7 $\{bankName\} \'b7 $\{today\}</p>\
  </div>\
  <div class="cards">\
  ...
  $\{rowsHtml\}\
  <div class="footer">Reporte generado por el Agente de Mau Bautista \'b7 V&M Bookkeeping Group LLC</div>\
</body>\
</html>`;\
    const win = window.open("", "_blank");\
    win.document.write(html);\
    ...
    <div style=\{S.app\}>\
      ...
      \{screen==="home"&&(\
        <div style=\{S.page\}>\
          ...
          <div style=\{S.card\}>\
            <h2 style=\{\{fontSize:15,fontWeight:700,marginBottom:14\}\}>\uc0\u10133  Nuevo Cliente</h2>\
            ...
            <button style=\{\{...S.btn,...S.btnGold\}\} onClick=\{createClient\} disabled=\{!newName.trim()||!newType\}>Crear Cliente \uc0\u8594 </button>\
          </div>\
          \{clients.length>0&&(\
            <div style=\{S.card\}>\
              <h2 style=\{\{fontSize:15,fontWeight:700,marginBottom:14\}\}>\uc0\u55357 \u56513  Clientes (\{clients.length\})</h2>\
                  ...
                  return (\
                    <div key=\{c.id\} className="cc" style=\{\{padding:"16px 20px",border:"1px solid #e2e8f0",borderRadius:12,display:"flex",alignItems:"center",justifyContent:"space-between",background:"#fff",transition:"all 0.15s"\}\}>\
                      <div onClick=\{()=>selectClient(c.id)\} style=\{\{flex:1,cursor:"pointer"\}\}>\
                        ...
                        <div style=\{\{color:"#999",fontSize:13,marginTop:4\}\}>\{bt?.icon\} \{bt?.label\} \'b7 \uc0\u55358 \u56800  \{ml\} aprendidas \'b7 \u55357 \u56516  \{(c.history||[]).length\} procesados</div>\
                      </div>\
                      <div style=\{\{display:"flex",gap:8,alignItems:"center"\}\}>\
                        <button style=\{\{...S.btn,...S.btnPrimary,fontSize:11,padding:"6px 14px"\}\} onClick=\{()=>selectClient(c.id)\}>Abrir \uc0\u8594 </button>\
                        <button style=\{\{...S.btn,background:"#fee2e2",color:"#991b1b",border:"1px solid #fca5a5",fontSize:11,padding:"6px 10px"\}\}\
                          onClick=\{async(e)=>\{\
                            e.stopPropagation();\
                            if(!window.confirm(`\'bfEliminar a $\{c.name\}? Esta acci\'f3n no se puede deshacer.`)) return;\
      ...
      \{screen==="upload"&&clientData&&(\
        <div style=\{S.page\}>\
          <div style=\{\{marginBottom:20\}\}>\
            ...
            <p style=\{S.sub\}>\{BUSINESS_TYPES.find(b=>b.id===clientData.businessType)?.icon\} \{BUSINESS_TYPES.find(b=>b.id===clientData.businessType)?.label\} \'b7 \uc0\u55358 \u56800  \{Object.keys(clientData.learnedMerchants||\{\}).length\} aprendidas</p>\
          </div>\
          <div style=\{S.card\}>\
            \{/* Split mode */\}\
            <div style=\{\{marginBottom:16,padding:"12px 16px",background:"rgba(26,86,219,0.1)",borderRadius:10,border:"1px solid rgba(26,86,219,0.3)"\}\}>\
              <div style=\{\{display:"flex",alignItems:"center",justifyContent:"space-between"\}\}>\
                <div>\
                  <div style=\{\{fontSize:13,fontWeight:700,color:"#1a1a1a"\}\}>\uc0\u55357 \u56516  Modo PDF Dividido</div>\
                  <div style=\{\{fontSize:11,color:"#64748b",marginTop:2\}\}>Para PDFs de 7+ p\'e1ginas \'97 procesa m\'faltiples partes y las junta autom\'e1ticamente</div>\
                </div>\
                <button onClick=\{()=>\{\
                ...
                <div style=\{\{marginTop:10,display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"\}\}>\
                    ...
                    return (\
                      <div\
                        ...
                        style=\{\{\
                          ...
                          position:"relative",\
                        \}\}>\
                        \{isDone && !isCurrent ? "\uc0\u10003 " : n\}\
                        \{isDone && !isCurrent && (\
                          <span style=\{\{position:"absolute",top:-4,right:-4,background:"#f59e0b",borderRadius:"50%",width:12,height:12,fontSize:8,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,color:"#000"\}\}>\uc0\u8634 </span>\
                        )\}\
                      </div>\
                    );\
                  \})\}\
                  \{splitParts.filter(Boolean).length>0&&(\
                    <div style=\{\{fontSize:11,color:"#22c55e",marginLeft:4,fontWeight:600\}\}>\
                      \uc0\u9989  \{splitParts.filter(Boolean).reduce((s,p)=>s+p.length,0)\} transacciones acumuladas\
                    </div>\
                  )\}\
                  \{splitParts.filter(Boolean).length>0&&(\
            ...
            <div className=\{`drop$\{dragOver?" over":""\}`\}\
              ...
              <div style=\{\{fontSize:36,marginBottom:10\}\}>\uc0\u55357 \u56516 </div>\
              \{file?(<><div style=\{\{fontWeight:600,color:"#1a56db"\}\}>\{file.name\}</div><div style=\{\{color:"#94a3b8",fontSize:12,marginTop:3\}\}>\{(file.size/1024).toFixed(0)\} KB</div></>)\
                   :(<><div style=\{\{fontWeight:500,color:"#1a1a1a"\}\}>Arrastra el bank statement aqu\'ed</div><div style=\{\{color:"#64748b",fontSize:12,marginTop:3\}\}>o click \'b7 Solo PDF bancario digital</div></>)\}\
            ...
            <div style=\{\{marginTop:12,padding:"10px 14px",background:"#fef3c7",borderRadius:8,border:"1px solid #fde68a",fontSize:12,color:"#92400e"\}\}>\
              <div style=\{\{fontWeight:700,marginBottom:4\}\}>\uc0\u55357 \u56523  Antes de subir tu PDF:</div>\
              <div>1. Elimina las p\'e1ginas de <strong>im\'e1genes de cheques</strong> \'97 ya est\'e1n capturados en la tabla de Checks</div>\
              <div>2. Elimina las p\'e1ginas en blanco ("This page intentionally left blank")</div>\
              <div>3. Para PDFs de 7+ p\'e1ginas, activa el <strong>Modo Split</strong> y divide en partes de m\'e1ximo 6 p\'e1ginas</div>\
            </div>\
            \{/* \uc0\u9472 \u9472  \u55357 \u57000  BANNER: PDF bloqueado \'97 aparece despu\'e9s de detecci\'f3n \u9472 \u9472  */\}\
            \{bankInfo && bankInfo.total_pages > 6 && !splitMode && (\
              <div style=\{\{marginTop:12,padding:"14px 16px",background:"#fee2e2",borderRadius:10,border:"2px solid #ef4444"\}\}>\
                <div style=\{\{fontWeight:700,color:"#991b1b",fontSize:14,marginBottom:4\}\}>\
                  \uc0\u55357 \u57000  PDF bloqueado \'97 \{bankInfo.total_pages\} p\'e1ginas detectadas\
                </div>\
                <div style=\{\{fontSize:12,color:"#7f1d1d",marginBottom:10\}\}>\
                  Este statement tiene demasiadas p\'e1ginas. Activa el <strong>Modo Split</strong> y divide el PDF en partes de m\'e1ximo 6 p\'e1ginas.\
                </div>\
                <button\
                ...
                <button style=\{\{...S.btn,...S.btnGold\}\} onClick=\{runExtraction\}>\
                  \{splitMode\
                    ? splitParts[splitPartNum-1]\
                      ? `\uc0\u55357 \u56580  Recargar Parte $\{splitPartNum\}`\
                      : `\uc0\u55357 \u56960  Procesar Parte $\{splitPartNum\}`\
                    : "\uc0\u55357 \u56960  Procesar"\}\
                </button>\
              </div>\
            )\}\
          ...
          <div style=\{S.card\}>\
            <div style=\{\{fontSize:12,fontWeight:700,marginBottom:10,color:"#94a3b8"\}\}>\uc0\u55356 \u57318  BIBLIOTECA DE BANCOS (\{KNOWN_BANKS.length\} bancos)</div>\
            <div style=\{\{display:"flex",flexWrap:"wrap",gap:6\}\}>\
              \{KNOWN_BANKS.map(b=>(\
                <span key=\{b\} style=\{\{background:"#dcfce7",color:"#166534",borderRadius:6,padding:"3px 10px",fontSize:11,fontWeight:600\}\}>\
                  \uc0\u9989  \{b.replace(/_/g," ").replace(/\\b\\w/g,c=>c.toUpperCase())\}\
                </span>\
              ))\}\
            </div>\
            <div style=\{\{fontSize:11,color:"#94a3b8",marginTop:8\}\}>\
              Si tu banco no aparece aqu\'ed, la app lo procesar\'e1 con el modo gen\'e9rico y te avisar\'e1 al finalizar.\
            </div>\
          </div>\
          \{Object.keys(clientData.learnedMerchants||\{\}).length>0&&(\
            <div style=\{S.card\}>\
              <div style=\{\{fontSize:12,fontWeight:700,marginBottom:10,color:"#94a3b8"\}\}>\uc0\u55358 \u56800  MEMORIA (\{Object.keys(clientData.learnedMerchants).length\} reglas)</div>\
              <div style=\{\{display:"flex",flexWrap:"wrap",gap:6\}\}>\
                \{Object.entries(clientData.learnedMerchants).map(([k,v])=>(\
                  <div key=\{k\} style=\{\{background:"#f4f6f9",border:"1px solid #e2e8f0",borderRadius:6,padding:"3px 9px",fontSize:11\}\}>\
                    <span style=\{\{fontWeight:700,color:"#1a1a1a"\}\}>\{k\}</span> \uc0\u8594  <span style=\{\{color:"#1a56db"\}\}>\{v\}</span>\
                  </div>\
                ))\}\
              </div>\
            </div>\
      ...
      \{/* \uc0\u9472 \u9472  EXTRACTING \u9472 \u9472  */\}\
      \{screen==="extracting"&&(()=>\{\
        const FRASES = [\
          "\uc0\u9749  Puedes ir por un caf\'e9... d\'e9jame trabajar a m\'ed.",\
          "\uc0\u55358 \u56595  Mientras trabajo, \'bfqu\'e9 te parece estar aprendiendo con Mau?",\
          "\uc0\u55357 \u56481  Dato curioso: esto antes te tomaba 3 horas. Ahora... unos segundos.",\
          "\uc0\u55357 \u56960  Mau dice: 'El que automatiza primero, gana primero.'",\
          "\uc0\u55358 \u56800  Estoy leyendo cada l\'ednea del estado de cuenta. T\'fa rel\'e1jate.",\
          "\uc0\u55357 \u56522  Bookkeeping inteligente. As\'ed se hace en el siglo 21.",\
          "\uc0\u55356 \u57263  Categor\'edas, saldos, cheques... nada se me escapa.",\
          "\uc0\u55357 \u56846  Tranquilo, yo soy m\'e1s r\'e1pido que cualquier contador manual.",\
          "\uc0\u55357 \u56496  Cada centavo importa. Por eso los cuento todos.",\
          "\uc0\u55356 \u57134  \'bfYa comiste? Yo trabajo mejor que t\'fa con hambre.",\
          "\uc0\u55356 \u57303 \u65039  Construyendo tu reporte... ladrillo por ladrillo.",\
          "\uc0\u9889  Si esto fuera manual, ya ibas en la p\'e1gina 2. Yo voy en la 14.",\
          "\uc0\u55356 \u57235  Mau ense\'f1a, yo aprendo, t\'fa ganas. Equipo perfecto.",\
          "\uc0\u55357 \u56561  Puedes revisar Instagram... ya yo me encargo aqu\'ed.",\
          "\uc0\u55358 \u56605  Mientras yo analizo, piensa en tu pr\'f3ximo cliente.",\
        ];\
        return (\
      ...
      \})()\}\
      \{/* \uc0\u9472 \u9472  RECONCILE \u9472 \u9472  */\}\
      \{screen==="reconcile"&&(\
        <div style=\{S.page\}>\
          <div style=\{\{marginBottom:20,display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:10\}\}>\
            <div>\
              <h1 style=\{S.h1\}>\uc0\u9878 \u65039  Conciliaci\'f3n de Saldos</h1>\
              <p style=\{S.sub\}>\
                \{splitMode && splitParts.length > 0\
                  ? `Modo PDF dividido \'97 $\{splitParts.length\} parte(s) \'b7 $\{transactions.length\} transacciones totales`\
                  : "Verifica que los saldos del banco cuadren con las transacciones extra\'eddas"\
                \}\
              </p>\
              \{splitMode && (\
                <div style=\{\{display:"flex",gap:8,marginTop:8,flexWrap:"wrap"\}\}>\
                  <button style=\{\{...S.btn,background:"#1a56db",color:"#fff",fontSize:11,padding:"5px 14px"\}\} onClick=\{()=>setScreen("upload")\}>\
                    \uc0\u10133  Agregar Parte \{splitPartNum\}\
                  </button>\
                  <button\
                    ...
                    \{runningFinalPass ? "\uc0\u9203  Procesando..." : "\u9989  Son todas las partes"\}\
                  </button>\
                </div>\
              )\}\
            </div>\
            ...
            <div style=\{\{...S.card, marginBottom:14, padding:"14px 18px"\}\}>\
              <div style=\{\{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8\}\}>\
                <div style=\{\{display:"flex",alignItems:"center",gap:10\}\}>\
                  <span style=\{\{fontSize:20\}\}>\uc0\u55356 \u57318 </span>\
                  <div>\
                    <div style=\{\{fontWeight:700,fontSize:14,color:"#1a1a1a"\}\}>\{bankInfo.bank_name\}</div>\
                    <div style=\{\{fontSize:11,color:"#64748b"\}\}>\
                      \{bankInfo.total_pages > 0 ? `$\{bankInfo.total_pages\} p\'e1ginas \'b7 ` : ""\}\
                      \{bankInfo.period_start\} \'96 \{bankInfo.period_end\}\
                    </div>\
                  </div>\
                </div>\
                \{bankInfo.bank_id === "unknown" ? (\
                  <span style=\{\{background:"#fef3c7",color:"#92400e",borderRadius:8,padding:"4px 12px",fontSize:11,fontWeight:700\}\}>\
                    \uc0\u9888 \u65039  Banco no identificado \'97 prompt gen\'e9rico\
                  </span>\
                ) : (\
                  <span style=\{\{background:"#dcfce7",color:"#166534",borderRadius:8,padding:"4px 12px",fontSize:11,fontWeight:700\}\}>\
                    \uc0\u9989  \{bankInfo.bank_id.replace(/_/g," ").replace(/\\b\\w/g,c=>c.toUpperCase())\}\
                  </span>\
                )\}\
              </div>\
              \{bankInfo.bank_id === "unknown" && (\
                <div style=\{\{marginTop:10,padding:"10px 14px",background:"#fef3c7",borderRadius:8,border:"1px solid #fde68a",fontSize:12,color:"#92400e"\}\}>\
                  \uc0\u9888 \u65039  <strong>Banco no encontrado en la biblioteca.</strong> Se proces\'f3 con el modo gen\'e9rico \'97 puede haber diferencias.\
                  <br/>\
                  <span style=\{\{fontSize:11,marginTop:4,display:"block"\}\}>\
                    \uc0\u55357 \u56524  Para mejorar la precisi\'f3n: notifica a Mau con el nombre del banco <strong>"\{bankInfo.bank_name\}"</strong> para agregarlo a la biblioteca.\
                  </span>\
                </div>\
              )\}\
              \{bankInfo.total_pages > 6 && !splitMode && (\
                <div style=\{\{marginTop:10,padding:"10px 14px",background:"#e0f2fe",borderRadius:8,border:"1px solid #7dd3fc",fontSize:12,color:"#0369a1"\}\}>\
                  \uc0\u55357 \u56481  <strong>PDF de \{bankInfo.total_pages\} p\'e1ginas detectado.</strong> Para mejor precisi\'f3n, activa el <strong>Modo PDF Dividido</strong> \'97 divide el PDF en partes de 6 p\'e1ginas m\'e1ximo. <strong>Importante:</strong> si tu banco incluye im\'e1genes de cheques (Arvest, Bank of Oklahoma, Mabrey), elimina esas p\'e1ginas antes de subir \'97 solo sube hasta la p\'e1gina del Daily Account Balance.\
                </div>\
              )\}\
            </div>\
          )\}\
          <div style=\{\{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap"\}\}>\
            \{[\
              ...
              \{l:"Total Dep\'f3sitos",v:`$$\{fmt(totalDepositsAmt)\}`,c:"#22c55e"\},\
          ...
          \{balances.length>0 ? (\
            <div style=\{\{...S.card,padding:0,overflow:"hidden"\}\}>\
              <div style=\{\{background:"#0f1f4b",color:"#fff",padding:"10px 16px",fontSize:12,fontWeight:700,letterSpacing:1\}\}>\
                \uc0\u55357 \u56522  CONCILIACI\'d3N POR CUENTA\
              </div>\
              <div style=\{\{display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr 1fr 1fr 1fr 80px",padding:"8px 16px",background:"#f7f6f2",borderBottom:"1px solid #e2e8f0"\}\}>\
                \{["CUENTA","PER\'cdODO","SALDO INICIAL","+ DEP\'d3SITOS","- RETIROS","SALDO FINAL","STATUS"].map(h=>(\
                ...
                return (\
                  <div key=\{i\} style=\{\{borderBottom:"1px solid #f0ede8"\}\}>\
                    <div style=\{\{display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr 1fr 1fr 1fr 80px",padding:"12px 16px",alignItems:"center",background:ok?"#fff":"#fff8f8"\}\}>\
                      ...
                      <div style=\{\{fontSize:11,color:"#64748b"\}\}>\{b.period_start||"\'97"\}<br/>\{b.period_end||""\}</div>\
                      ...
                      <div style=\{\{textAlign:"center"\}\}>\
                        \{ok\
                          ? <span style=\{\{background:"#dcfce7",color:"#166534",borderRadius:6,padding:"3px 8px",fontSize:11,fontWeight:700\}\}>\uc0\u9989  OK</span>\
                          : <span style=\{\{background:"#fee2e2",color:"#991b1b",borderRadius:6,padding:"3px 8px",fontSize:11,fontWeight:700\}\}>\uc0\u9888 \u65039  DIF</span>\
                        \}\
                      </div>\
                    </div>\
                    \{!ok&&(\
                      <div style=\{\{background:"#fef3c7",padding:"6px 16px",fontSize:11,color:"#92400e",borderTop:"1px solid #fde68a"\}\}>\
                        \uc0\u9888 \u65039  Diferencia de $\{fmt(diff)\} \'97 Revisa si hay transacciones faltantes\
                      </div>\
                    )\}\
                  </div>\
                );\
            ...
            <div style=\{\{...S.card,textAlign:"center",padding:"32px 20px"\}\}>\
              <div style=\{\{fontSize:32,marginBottom:10\}\}>\uc0\u55357 \u56589 </div>\
          ...
          \{balances.length>0&&(\
            <div style=\{\{...S.card,marginTop:14\}\}>\
              <div style=\{\{fontSize:12,fontWeight:700,color:"#64748b",marginBottom:12,letterSpacing:1\}\}>\uc0\u55357 \u56590  BANCO vs EXTRA\'cdDO</div>\
              \{(()=>\{\
                ...
                if (isMSU) \{\
                  return (\
                    <div style=\{\{padding:"12px 14px",background:"#f0f9ff",borderRadius:8,border:"1px solid #bae6fd",fontSize:12,color:"#0369a1"\}\}>\
                      \uc0\u8505 \u65039  MSU FCU \'97 La conciliaci\'f3n se verifica en la tabla de saldos por cuenta arriba. El balance de cada subcuenta (SAVER, IMMA, CHECKING) est\'e1 validado individualmente.\
                    </div>\
                  );\
                \}\
                // L\'f3gica est\'e1ndar para otros bancos\
                  ...
                  <>\
                    \{isFiltered && (\
                      <div style=\{\{fontSize:11,color:"#64748b",marginBottom:10,padding:"6px 10px",background:"#f0f9ff",borderRadius:6,border:"1px solid #bae6fd"\}\}>\
                        \uc0\u8505 \u65039  Comparando solo <strong>\{compareBals[0]?.account_name\}</strong> \'97 subcuentas conciliadas individualmente arriba\
                      </div>\
                    )\}\
                    <div style=\{\{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:12\}\}>\
                      \{[\
                        \{l:"Dep\'f3sitos Banco",   v:`$$\{fmt(bankDep)\}`,            c:"#22c55e"\},\
                        \{l:"Dep\'f3sitos Extra\'eddos",v:`$$\{fmt(totalDepositsAmt)\}`,  c:"#22c55e"\},\
                        \{l:"Retiros Banco",      v:`$$\{fmt(bankWith)\}`,           c:"#ef4444"\},\
                        \{l:"Retiros Extra\'eddos",  v:`$$\{fmt(totalWithdrawalsAmt)\}`,c:"#ef4444"\},\
                      ].map(s=>(\
                    ...
                    <div style=\{\{marginTop:12,padding:"10px 14px",borderRadius:8,background:allGood?"#dcfce7":"#fef3c7",border:`1px solid $\{allGood?"#86efac":"#fde68a"\}`\}\}>\
                      \{allGood\
                        ? <span style=\{\{color:"#166534",fontWeight:600,fontSize:13\}\}>\uc0\u9989  Todo cuadra \'97 Las transacciones coinciden con los totales del banco</span>\
                        : <span style=\{\{color:"#92400e",fontWeight:600,fontSize:13\}\}>\uc0\u9888 \u65039  Posible diferencia \'97 Dep\'f3sitos: $\{fmt(depDiff)\} \'b7 Retiros: $\{fmt(withDiff)\}</span>\
                      \}\
                    </div>\
                  </>\
                );\
          ...
          <div style=\{\{display:"flex",justifyContent:"flex-end",marginTop:16,gap:10\}\}>\
            <button style=\{\{...S.btn,...S.btnOutline,color:"#fff",background:"#1a56db",borderColor:"#1a56db"\}\} onClick=\{()=>setScreen("upload")\}>\uc0\u8592  Volver</button>\
            ...
            \}\}>\
              \{(()=>\{\
                ...
                const extDep   = transactions.filter(r=>r.type==="DEPOSIT").reduce((s,r)=>s+Math.abs(parseFloat(r.amount)||0),0);\
                const hasDiff  = Math.abs(bankWith-extWith)>50 || Math.abs(bankDep-extDep)>50;\
                if (hasDiff) return "\uc0\u55357 \u56615  Corregir duplicados \u8594 ";\
                return askQueue.length>0 ? `Resolver $\{askQueue.length\} ambig\'fcedades \uc0\u8594 ` : "Ver transacciones \u8594 ";\
              \})()\}\
            </button>\
          </div>\
        </div>\
      ...
      \{screen==="dedup"&&(()=>\{\
        ...
        const dupConcepts = dupKeys.map(k => \{\
          ...
          return `$\{concept\} ($$\{fmt(parseFloat(amt))\}) \'97 $\{date\}`;\
        \});\
        const groups = \{\};\
        dupKeys.forEach(k => \{\
          const [concept, amt, date] = k.split("||");\
          const displayKey = `$\{concept\} ($$\{fmt(parseFloat(amt))\}) \'97 $\{date\}`;\
        ...
        return (\
          <div style=\{S.page\}>\
            <div style=\{\{marginBottom:20,display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:10\}\}>\
              <div>\
                <h1 style=\{S.h1\}>\uc0\u55357 \u56615  Corrector de Duplicados</h1>\
              ...
              <div style=\{\{display:"flex",gap:8\}\}>\
                <button style=\{\{...S.btn,...S.btnOutline,fontSize:12,color:"#fff",background:"#64748b",border:"none"\}\}\
                  onClick=\{()=>setScreen("reconcile")\}>\uc0\u8592  Volver</button>\
            ...
            \{dupConcepts.length === 0 ? (\
              <div style=\{\{...S.card,textAlign:"center",padding:"40px 20px"\}\}>\
                <div style=\{\{fontSize:32,marginBottom:10\}\}>\uc0\u9989 </div>\
                ...
                <button style=\{\{...S.btn,...S.btnGold,marginTop:16\}\} onClick=\{()=>setScreen(askQueue.length>0?"resolve":"review")\}>\
                  Continuar de todas formas \uc0\u8594 \
                </button>\
              </div>\
            ) : (\
              <>\
                <div style=\{\{...S.card,padding:0,overflow:"hidden",marginBottom:14\}\}>\
                  <div style=\{\{background:"#0f1f4b",color:"#fff",padding:"10px 16px",fontSize:12,fontWeight:700,letterSpacing:1,display:"flex",justifyContent:"space-between",alignItems:"center"\}\}>\
                    <span>\uc0\u55357 \u56523  CONCEPTOS DUPLICADOS (\{dupConcepts.length\} grupos)</span>\
                    <div style=\{\{display:"flex",gap:8\}\}>\
                            ...
                            return (\
                              <div key=\{key\} style=\{\{display:"flex",alignItems:"center",gap:12,padding:"9px 16px 9px 32px",background:isMarked?"#fff8f8":isFirst?"#f0fdf4":"#fff",borderTop:"1px solid #f1f5f9",transition:"background 0.15s"\}\}>\
                                ...
                                <div style=\{\{fontSize:10,padding:"2px 8px",borderRadius:4,fontWeight:700,\
                                  ...
                                  \{isFirst ? "\uc0\u10003  ORIGINAL" : "\u9888  DUPLICADO"\}\
                                </div>\
                              </div>\
                            );\
                          \})\}\
                ...
                <div style=\{\{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10\}\}>\
                  <div style=\{\{fontSize:13,color:"#64748b"\}\}>\
                    \{markedCount > 0\
                      ? `$\{markedCount\} transacciones marcadas \'97 se eliminar\'e1n $$\{fmt(markedAmt)\} de retiros`\
                  ...
                  <div style=\{\{display:"flex",gap:8\}\}>\
                    ...
                    <button\
                      ...
                      onClick=\{()=>\{\
                        ...
                        applyDedup(keys);\
                      \}\}>\
                      \uc0\u9989  Aplicar y recalcular (\{markedCount\})\
                    </button>\
                  </div>\
                </div>\
              </>\
        ...
        return (\
          <div style=\{S.page\}>\
            <div style=\{\{marginBottom:20,display:"flex",justifyContent:"space-between",alignItems:"flex-start"\}\}>\
              <div><h1 style=\{S.h1\}>Resolver Ambig\'fcedades</h1><p style=\{S.sub\}>\{currentAsk+1\} de \{askQueue.length\}</p></div>\
              <button style=\{\{...S.btn,...S.btnOutline,fontSize:12,color:"#fff",background:"#1a56db",borderColor:"#1a56db"\}\} onClick=\{()=>setScreen("review")\}>Saltar todos \uc0\u8594 </button>\
            </div>\
            ...
            <div style=\{S.card\}>\
              <div style=\{\{background:"#f4f6f9",borderRadius:8,padding:14,marginBottom:16\}\}>\
                ...
                \{ask.reason&&<div style=\{\{marginTop:8,fontSize:12,color:"#888"\}\}>\uc0\u55357 \u56481  \{ask.reason\}</div>\}\
              </div>\
              \{isZelle&&!isDeposit&&zelleName&&(\
                <div style=\{\{marginBottom:16\}\}>\
                  <span style=\{S.label\}>ZELLE A: <strong style=\{\{color:"#1a1a1a"\}\}>\{zelleName\}</strong></span>\
                  <div style=\{\{display:"flex",gap:6,flexWrap:"wrap",marginTop:8\}\}>\
                    \{[\{l:"\uc0\u55357 \u56439  Subcontractor",c:"Subcontractor Expense"\},\{l:"\u55357 \u56508  Payroll",c:"Payroll & Wages"\},\{l:"\u55356 \u57312  Owner Draw",c:"Owner Draw"\},\{l:"\u55357 \u56420  Personal Payment",c:"Personal Payment"\},\{l:"\u55357 \u56911  Donations",c:"Donations"\},\{l:"\u55357 \u56550  Materials",c:"Materials"\},\{l:"\u55357 \u56580  Transfer Out",c:"Transfer Out"\},\{l:"\u55356 \u57213 \u65039  Meals",c:"Meals & Entertainment"\}].map(o=>(\
                      <button key=\{o.c\} className="ropt" onClick=\{()=>resolveAsk(o.c,true,zelleName)\}>\{o.l\}</button>\
                    ))\}\
                  </div>\
                  <div style=\{\{fontSize:11,color:"#bbb",marginTop:5\}\}>\uc0\u10003  Se guardar\'e1 en memoria: "\{zelleName\}"</div>\
                </div>\
              )\}\
              \{isZelle&&isDeposit&&zelleName&&(\
                <div style=\{\{marginBottom:16\}\}>\
                  ...
                  <div style=\{\{display:"flex",gap:6,flexWrap:"wrap",marginTop:8\}\}>\
                    \{[\{l:"\uc0\u55357 \u56496  Income Services",c:"Income - Services"\},\{l:"\u55356 \u57318  Owner Investment",c:"Owner Investment"\},\{l:"\u55357 \u56580  Transfer In",c:"Transfer In"\},\{l:"\u55357 \u56548  Loan Proceeds",c:"Loan Proceeds"\}].map(o=>(\
                      <button key=\{o.c\} className="ropt" onClick=\{()=>resolveAsk(o.c,true,zelleName)\}>\{o.l\}</button>\
                    ))\}\
                  </div>\
                </div>\
              ...
              <div style=\{\{marginBottom:14\}\}>\
                <span style=\{S.label\}>CATEGOR\'cdA MANUAL:</span>\
      ...
      \{screen==="review"&&(\
        <div style=\{S.page\}>\
          ...
          \{askPct>15&&<div style=\{\{background:"#fef3c7",border:"1px solid #f59e0b",borderRadius:8,padding:"9px 14px",marginBottom:12,fontSize:12,color:"#92400e"\}\}>\uc0\u9888 \u65039  <strong>ALERTA:</strong> \{askPct\}% supera el l\'edmite de 15%</div>\}\
          <div style=\{\{display:"flex",gap:7,marginBottom:12,flexWrap:"wrap",alignItems:"center",justifyContent:"space-between"\}\}>\
            <div style=\{\{display:"flex",gap:6,flexWrap:"wrap"\}\}>\
              <button style=\{\{...S.btn,background:"#edf2f7",color:"#1a1a1a",fontSize:11,padding:"6px 12px"\}\} onClick=\{()=>setScreen("reconcile")\}>\uc0\u9878 \u65039  Conciliaci\'f3n</button>\
            </div>\
            <div style=\{\{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"\}\}>\
              <div style=\{\{display:"flex",gap:0,alignItems:"center",border:"1px solid #1a56db",borderRadius:10,overflow:"hidden",background:"#fff"\}\}>\
                <select value=\{selectedCat\} onChange=\{e=>setSelectedCat(e.target.value)\}\
                  ...
                  <option value="">\'97 Elegir categor\'eda \'97</option>\
                ...
                <button onClick=\{()=>downloadByCategory(selectedCat)\} disabled=\{!selectedCat\}\
                  ...
                  \uc0\u11015  Por Categor\'eda\
                </button>\
              </div>\
              <button style=\{\{...S.btn,...S.btnOutline,fontSize:11,color:"#fff",background:"#1a56db",borderColor:"#1a56db"\}\} onClick=\{()=>downloadWave("DEPOSIT")\}>\uc0\u11015  Wave DEPOSITS</button>\
              <button style=\{\{...S.btn,...S.btnOutline,fontSize:11,color:"#fff",background:"#1a56db",borderColor:"#1a56db"\}\} onClick=\{()=>downloadWave("WITHDRAWAL")\}>\uc0\u11015  Wave WITHDRAWALS</button>\
              <button style=\{\{...S.btn,...S.btnPrimary,fontSize:11\}\} onClick=\{downloadCSV\}>\uc0\u11015  CSV Completo</button>\
              <button style=\{\{...S.btn,background:"#166534",color:"#fff",fontSize:11,fontWeight:700\}\} onClick=\{downloadPnL\}>\uc0\u55357 \u56522  P&L + Personales</button>\
              <button style=\{\{...S.btn,...S.btnGold\}\} onClick=\{finalize\}>\uc0\u10003  Finalizar</button>\
            </div>\
          </div>\
          \{deposits.length>0&&(\
            <div style=\{\{...S.card,padding:0,overflow:"hidden",marginBottom:12\}\}>\
              <div style=\{\{background:"#166534",color:"#fff",padding:"8px 14px",fontSize:11,fontWeight:700,letterSpacing:1\}\}>\uc0\u9650  DEPOSITS (\{deposits.length\})</div>\
          ...
          \{withdrawals.length>0&&(\
            <div style=\{\{...S.card,padding:0,overflow:"hidden",marginBottom:12\}\}>\
              <div style=\{\{background:"#991b1b",color:"#fff",padding:"8px 14px",fontSize:11,fontWeight:700,letterSpacing:1\}\}>\uc0\u9660  WITHDRAWALS (\{withdrawals.length\})</div>\
          ...
          \{checkReportRows.length>0&&(\
            <div style=\{\{...S.card,padding:0,overflow:"hidden",marginBottom:12\}\}>\
              <div style=\{\{background:"#0f1f4b",color:"#fff",padding:"8px 14px",fontSize:11,fontWeight:700,letterSpacing:1\}\}>\
                \uc0\u55357 \u56523  REPORTE DE CHEQUES (\{checks.length\} cheques \'b7 Total: $\{fmt(checks.reduce((s,r)=>s+Math.abs(parseFloat(r.amount)||0),0))\})\
              </div>\
              <div>\
      ...
      \{screen==="done"&&(\
        <div style=\{\{...S.page,textAlign:"center",paddingTop:60\}\}>\
          <div style=\{\{fontSize:52,marginBottom:14\}\}>\uc0\u9989 </div>\
          <h1 style=\{S.h1\}>\'a1Statement completado!</h1>\
          <p style=\{\{color:"#888",marginTop:6,marginBottom:10,fontSize:13\}\}>\
            \{transactions.length\} transacciones \'b7 \uc0\u55357 \u56580  \{transfers.length\} transfers \'b7 \u55358 \u56800  \{Object.keys(clientData?.learnedMerchants||\{\}).length\} en memoria\
            \{bankInfo?.bank_name ? ` \'b7 \uc0\u55356 \u57318  $\{bankInfo.bank_name\}` : ""\}\
          </p>\
      ...
      \{showDevModal && (()=>\{\
        ...
        const fullCode = Object.keys(allRules).length > 0\
          ? `// Reglas aprendidas - $\{today2\}\\n// Cliente: $\{clientData?.name || ""\}\\n// Tipo de negocio: $\{clientData?.businessType || ""\}\\n// Pegar dentro de MERCHANT_DICT en App.jsx \'97 buscar secci\'f3n del tipo de negocio\\n\\n$\{codeLines\}`\
          : `// Sin reglas aprendidas a\'fan\\n// Procesa un statement y resuelve las ambig\'fcedades primero`;\
        return (\
          <div style=\{\{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:24\}\}\
            ...
            <div style=\{\{background:"#0f1f4b",border:"1px solid #1a56db",borderRadius:16,padding:28,maxWidth:640,width:"100%",maxHeight:"80vh",overflow:"hidden",display:"flex",flexDirection:"column"\}\}\
              ...
              <div style=\{\{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16\}\}>\
                <div>\
                  <div style=\{\{color:"#fff",fontWeight:700,fontSize:16\}\}>\uc0\u55358 \u56800  Reglas Aprendidas</div>\
                  <div style=\{\{color:"#94a3b8",fontSize:12,marginTop:2\}\}>\{Object.keys(allRules).length\} reglas \'b7 \{clientData?.name\}</div>\
                </div>\
                <button onClick=\{()=>setShowDevModal(false)\}\
                  style=\{\{background:"transparent",border:"none",color:"#94a3b8",fontSize:20,cursor:"pointer",lineHeight:1\}\}>\uc0\u10005 </button>\
              </div>\
              ...
              <div style=\{\{display:"flex",gap:8,marginTop:14,justifyContent:"flex-end"\}\}>\
                <div style=\{\{fontSize:11,color:"#64748b",alignSelf:"center",flex:1\}\}>\
                  Copia este c\'f3digo y p\'e9galo en MERCHANT_DICT\
                </div>\
                <button\
                  onClick=\{()=>\{\
                  ...
                  \{copied ? "\uc0\u9989  Copiado!" : "\u55357 \u56523  Copiar c\'f3digo"\}\
                </button>\
              </div>\
            </div>\
          </div>\
...
\}\
// \uc0\u9472 \u9472  FRASE ROTATIVA \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \
...
\}\
// \uc0\u9472 \u9472  ANIMATED PERCENT \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \
